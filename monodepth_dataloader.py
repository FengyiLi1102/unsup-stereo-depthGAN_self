...
class MonodepthDataloader(object):
    ...
    # dataset: only relevant to the cityscapes
    def __init__(self, data_path, filenames_file, params, dataset, mode):
        ...
        if mode == 'train':
            ...
            # randomly flip images
            do_flip = tf.random_uniform([], 0, 1)
            left_image = tf.cond(do_flip > 2, lambda: tf.image.flip_left_right(right_image_o), lambda: left_image_o)
            right_image = tf.cond(do_flip > 2, lambda: tf.image.flip_left_right(left_image_o), lambda: right_image_o)

            # randomly augment images
            do_augment = tf.random_uniform([], 0, 1)
            left_image, right_image = tf.cond(do_augment > 2, lambda: self.augment_image_pair(left_image, right_image),
                                              lambda: (left_image, right_image))
            ...
            min_after_dequeue = 2048
            capacity = min_after_dequeue + 4 * params.batch_size
            self.left_image_batch, self.right_image_batch = tf.train.batch([left_image, right_image], params.batch_size)
